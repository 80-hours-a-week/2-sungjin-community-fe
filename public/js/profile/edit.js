...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    nicknameInput.addEventListener('input', () => {
        ...
        submitButton.style.background = validateNickname(nickname) ? '#7F6AEE' : '#ACA0EB';
    });
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const updatedUser = response && response.data ? response.data : null;
            if (updatedUser) {
...
async function loadUserProfile() {
    try {
        ...
        const resolvedImageUrl = resolveImageUrl(user.profile_image_url);
        if (resolvedImageUrl && previewImage) previewImage.src = resolvedImageUrl;
        if (resolvedImageUrl && headerProfileImage) headerProfileImage.src = resolvedImageUrl;
    } catch (error) {
        handleApiError(error, {
